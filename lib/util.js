...
function assertNoUncommittedChanges(program) {
  return new Promise(
    function assertNoUncommittedChangesPromise(resolve, reject) {
      exec('git diff --quiet', function execCallback(error) {
        if (error) {
          return reject(new Error('You have uncommitted changes'));
        }
        exec('git diff --cached --quiet', function execCallback(error) {
          if (error) {
            return reject(new Error('You have uncommitted changes'));
          }
          resolve(program);
...
function getCurrentBranch() {
  return new Promise(function getCurrentBranchPromise(resolve, reject) {
    exec('git rev-parse --abbrev-ref HEAD', function execCallback(error, stdout) {
      if (error) {
        return reject(error);
      }
      resolve(stdout);
