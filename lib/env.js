...
function addCommandLineOptionForVariable(program, variable) {
  ...
  program.option(commandLineOption,
    'assign the ' + variable + ' environment variable');
  return program;
}
 ...
 * @param {object} program
 * @param {object} config
 * @param {?Map<string, *>} [excluding]
 * @param {?Set<string>} [unassigned]
 * @returns {object}
 */
function addCommandLineOptionsForVariablesUnassignedInConfigPlans(program, config, excluding, unassigned) {
  return addCommandLineOptionsForVariables(program,
    getVariablesUnassignedInConfigPlans(config, excluding, unassigned));
}
...
function getVariablesAssignedByProgram(program, variableNames, variables) {
  ...
  variableNames.forEach(function forEachVariableName(variableName) {
    var camelCased = camelCaseVariable(variableName);
  ...
  if (config.env) {
    for (var variable in config.env) {
      if (!variables.has(variable)) {
        variables.set(variable, config.env[variable]);
      }
    }
  }
  return variables;
...
function getVariablesRequiredByCommand(command, required) {
  ...
  if (matches) {
    matches.forEach(function forEachMatch(match) {
      var variable = match.match(/^\${?([a-zA-Z0-9_]+)}?$/)[1];
 ...
 * @param {object} config
 * @param {string} planName
 * @params {?Map<string, *>} [excluding]
 * @param {?Set<string>} [unassigned]
 * @returns {Set<string>}
 */
function getVariablesUnassignedInConfigPlan(config, planName, excluding, unassigned) {
 ...
 * @returns {Set<string>}
 */
function getVariablesUnassignedInConfigPlans(config, excluding, unassigned) {
  if (config.plans) {
    for (var planName in config.plans) {
      unassigned = getVariablesUnassignedInConfigPlan(config, planName,
