...
function getReleaseOrReleaseCandidateVersion(program) {
  ...
  return
    util.lift(util.assertValidReleaseOrReleaseCandidateVersion)(releaseVersion)
      .then(util.assertTagDoesNotExist)
...
function getConfirmation(program) {
  ...
  program.releasePlan = plan.releasePlan(program.releaseVersion);
  console.log('%s\n', program.releasePlan);
  ...
  if (program.nextDevelopmentVersion) {
    console.log([
      ...
      'Then, in order to continue the next Development Version, this tool ' +
        'will',
      'instruct the build server to update the version number in package.json',
      '(and any bower.json)',
      '',
      '    ' + colors.red(program.releaseVersion) + ' → ' +
    ...
    program.developmentVersionPlan =
      plan.developmentVersionPlan(program.nextDevelopmentVersion);
    console.log('%s\n', program.developmentVersionPlan);
  }
...
function triggerOrRunLocally(program) {
  if (program.server || program.local) {
    ...
    return program.releasePlan.run().then(function releasePlanResolved() {
      if (!program.developmentVersionPlan) {
        return program;
      }
      console.log(colors.underline('\nContinuing on Development Version…'));
      return program.developmentVersionPlan.run()
        .then(function developmentVersionPlanResolved() {
          return program;
        });
    });
  }
